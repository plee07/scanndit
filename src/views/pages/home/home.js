import Postwidget from '/src/views/components/postwidget/postwidget.js';
import Postfeed from '/src/views/components/postfeed/postfeed.js';

let Home = {
  render : async function () {
    let postwidget = await Postwidget.render();
  ...
  afterRender: async function () {
    Postwidget.afterRender();
    Postfeed.afterRender();
    $(window).scroll(function() {
      if($(window).scrollTop() + $(window).height() == $(document).height()) {
        getAllPosts()
          .then(data => Postfeed.appendToFeed(data, window.pagesDisplayed + 1));
      }
    });
